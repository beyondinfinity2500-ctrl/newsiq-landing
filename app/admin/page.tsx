    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <header className="sticky top-0 z-30 border-b border-border/80 bg-background/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 lg:px-8">
          <a href="/" className="flex items-center gap-3" aria-label="Back to NEWSiQ home"><span className="brand-mark">N</span><span className="font-mono text-sm font-bold tracking-[0.28em]">NEWS<span className="text-primary">iQ</span></span></a>
          <span className="rounded-full border border-primary/30 bg-primary/10 px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.14em] text-primary">Admin workspace</span>
        </div>
      </header>
      <div className="relative z-10 mx-auto grid max-w-7xl gap-8 px-4 py-8 lg:grid-cols-[minmax(0,680px)_320px] lg:px-8 lg:py-12">
        <section>
          <div className="mb-8"><p className="font-mono text-[10px] uppercase tracking-[0.2em] text-primary">Editorial console / New post</p><h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em]">Publish a short news update</h1><p className="mt-3 max-w-xl text-sm leading-6 text-muted-foreground">Create one verified update in English and its local-language version. The public feed remains admin-only.</p></div>
          <form onSubmit={handleSubmit} className="signal-panel space-y-6 p-5 sm:p-8">
            <div className="grid gap-5 sm:grid-cols-2"><label className="space-y-2"><span className="field-label">Region</span><input required placeholder="Indonesia" className="field-input" /></label><label className="space-y-2"><span className="field-label">Category</span><select className="field-input"><option>World</option><option>Markets</option><option>Geopolitics</option><option>Technology</option><option>Disaster</option></select></label></div>
            ...
            <label className="block space-y-2"><span className="field-label">Local headline</span><input required placeholder="Gempa kuat melanda Indonesia bagian timur" className="field-input text-base" /></label>
