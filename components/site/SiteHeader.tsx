  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border bg-background shadow-[0_8px_24px_hsl(var(--background)/.45)]">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3.5 lg:px-8">
        ...
        {!admin && <nav className="hidden items-center gap-0.5 xl:flex" aria-label="Primary navigation"><a href="/" className="rounded-md px-2.5 py-2 text-xs font-medium text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground">Home</a><a href="/about" className="rounded-md px-2.5 py-2 text-xs font-medium text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground">About</a><span className="inline-flex items-center gap-0.5"><button type="button" onClick={() => setLive((value) => !value)} className="live-toggle" aria-pressed={live} aria-label={`${live ? 'Disable' : 'Enable'} live market data`}><span className={`live-dot ${live ? '' : 'live-dot-off'}`} />{live ? 'Live' : 'Paused'}</button><a href="/live" className="rounded-md px-2.5 py-2 text-xs font-medium text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground">Live Feed</a><a href="/markets" className="rounded-md px-2.5 py-2 text-xs font-medium text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground">Markets</a></span>{newsCategories.filter((item) => item !== 'Markets').map((item) => <a key={item} href="#feed" className="rounded-md px-2.5 py-2 text-xs font-medium text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground">{item}</a>)}<a href="/terms" className="rounded-md px-2.5 py-2 text-xs font-medium text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground">Terms</a><a href="/subscribe" className="rounded-md bg-primary px-3 py-2 text-xs font-bold text-primary-foreground transition-transform hover:-translate-y-0.5">Subscribe</a>
        </nav>}
