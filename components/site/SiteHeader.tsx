    ...
    <header className="sticky top-0 z-30 border-b border-border/80 bg-background/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3.5 lg:px-8">
        ...
        {!admin && <nav className="hidden items-center gap-1 md:flex" aria-label="Primary navigation">
          {newsCategories.map((item) => <a key={item} href={item === 'Live feed' ? '#feed' : `#${item.toLowerCase()}`} className="rounded-md px-3 py-2 text-xs font-medium text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground">{item}</a>)}
        </nav>}
      ...
      {open && <nav id="mobile-navigation" className="border-t border-border/70 px-4 py-3 md:hidden" aria-label="Mobile navigation">
        <div className="grid gap-1">{(admin ? ['Back to live feed'] : [...newsCategories, 'Subscribe']).map((item) => <a key={item} href={admin ? '/' : item === 'Live feed' ? '#feed' : item === 'Subscribe' ? '#subscribe' : `#${item.toLowerCase()}`} onClick={() => setOpen(false)} className="rounded-md px-3 py-3 text-sm text-muted-foreground hover:bg-secondary hover:text-foreground">{item}</a>)}</div>
      </nav>}
    </header>
  )
}
